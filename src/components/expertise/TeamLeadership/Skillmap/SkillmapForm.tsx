...
export default function SkillmapForm({ onDataChange }: { onDataChange: (data: any[]) => void }) {
  ...
  const isComplete = () => {
    ...
    for (const theme of requiredThemes) {
      for (const skill of themes[theme]) {
        const r = ratings[skill.name];
        if (!r || r.mastery === null || r.relevance === null) return false;
      }
    }
    return true;
  };
      ...
      {role &&
        (role === "Individuell" ? Object.entries(themes) : Object.entries(themes).filter(([theme]) => roleProfiles[role].includes(theme))).map(([theme, skills]) => (
          <div key={theme} className="space-y-4">
            ...
            {/* Labels nur einmal pro Thema */}
            <div className="grid grid-cols-2 gap-8 w-2/3 ml-auto mb-2">
            ...
            {skills.map((skill) => (
              <div key={skill.name} className="border-b pb-2">
                <div
                  className="flex justify-between items-start cursor-pointer"
                  onClick={() => setOpenSkill(openSkill === skill.name ? null : skill.name)}
                >
                  <span className="font-medium truncate" title={skill.name}>{skill.name}</span>
                ...
                {openSkill === skill.name && <p className="mt-2 text-sm text-gray-600">{skill.description}</p>}
              </div>
            ))}
          </div>
        ))}
        ...
        <div className="flex justify-center mt-8 gap-4">
          <button
            ...
            className={`px-6 py-3 rounded-xl font-medium transition ${
              isComplete() ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-300 text-gray-500 cursor-not-allowed"
            }`}
          >
            Map erstellen
          </button>
