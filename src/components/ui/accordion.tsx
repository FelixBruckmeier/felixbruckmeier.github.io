...
import { typography, colors, radii, transitions, icon, spacing } from "@/lib/tokens";
...
>(({ className, children, variant = "default", title, subtitle, ...props }, ref) => {
  ...
  return (
    <AccordionPrimitive.Item ref={ref} className={baseClasses} {...props}>
      {timelineDot}
      {variant === "default" ? content : <Tile className="w-full">{content}</Tile>}
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex w-full">
    ...
    >
      {children}
      <ChevronDown
        className={cn(icon.md, "shrink-0 transition-transform duration-200", colors.muted.text)}
      />
    </AccordionPrimitive.Trigger>
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <Body className={cn(spacing.pb4, "pt-0", className)}>{children}</Body>
  </AccordionPrimitive.Content>
));
